...
> = async (req, res) => {
  try {
    ...
    // Вместо создания записи сразу, отправляем задачу в RabbitMQ
    await sendToQueue('enrollments', { userId, courseId })

    res.status(200).json({
      status: 'success',
      message: 'Запрос на запись отправлен, обработка начнётся в фоне',
    })
  } catch (error) {
    res.status(500).json({
      status: 'error',
